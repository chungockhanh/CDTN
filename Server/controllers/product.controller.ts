...
const getProduct = async (req: Request, res: Response) => {
  ...
  if (req.jwtDecoded && req.jwtDecoded.roles[0] === 'Admin') {
    productDB = await ProductModel.findById(condition)
      .populate('category')
      .select({ __v: 0 })
      .lean()
  } else {
    productDB = await ProductModel.findOneAndUpdate(
      condition,
      { $inc: { view: 1 } },
      { new: true }
    )
...
const deleteProduct = async (req: Request, res: Response) => {
  ...
  const purchaseDB: any = await PurchaseModel.findOne({
    product: product_id,
  }).lean()
  if (purchaseDB) {
    throw new ErrorHandler(
      STATUS.CONFLICT,
      'Không thể xóa thấy sản phẩm do sản phẩm này có tồn tại đơn hàng'
    )
  } else {
...
const getProductImg = async (req: Request, res: Response) => {
  let condition = { _id: req.params.product_id }
  let productDB: any = null
  productDB = await ProductModel.findById(condition).select({ __v: 0 }).lean()
...
const deleteProductImg = async (req: Request, res: Response) => {
  let condition = { _id: req.params.product_id }
  let productDB: any = null
  productDB = await ProductModel.findById(condition).select({ __v: 0 }).lean()

  if (productDB) {
    const image = productDB.image
    const newImages: string[] = productDB.images
    const index = newImages.findIndex((img) => img === image)
    if (index > -1) {
    ...
    if (updatedProduct) {
      ...
      throw new ErrorHandler(STATUS.CONFLICT, 'Cập nhật thất bại')
    }
  } else {
    throw new ErrorHandler(STATUS.NOT_FOUND, 'Không tìm thấy sản phẩm')
  }
...
const deleteProductImgs = async (req: Request, res: Response) => {
  let condition = { _id: req.params.product_id }
  let productDB: any = null
  productDB = await ProductModel.findById(condition).select({ __v: 0 }).lean()
  ...
  if (productDB) {
    const image = productDB.image
    const images = productDB.images
    const index = images.findIndex((img) => img === image)
    if (index !== -1) {
      images.splice(index, 1)
    }
    const newImages: string[] = [image]
