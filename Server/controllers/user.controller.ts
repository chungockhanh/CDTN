...
const getUsers = async (req: Request, res: Response) => {
  ...
  let [users, totalUsers]: [users: any, totalUsers: any] = await Promise.all([
...
const updateUser = async (req: Request, res: Response) => {
  ...
  const emailUserDB = await UserModel.findOne({ email }).lean()
  if (emailUserDB && emailUserDB._id.toString() !== req.params.user_id) {
...
const updateMe = async (req: Request, res: Response) => {
  ...
  const emailUserDB = await UserModel.findOne({ email }).lean()
  if (emailUserDB && emailUserDB._id.toString() !== req.jwtDecoded.id) {
    throw new ErrorHandler(STATUS.UNPROCESSABLE_ENTITY, {
      email: 'Email đã tồn tại',
    })
  }
